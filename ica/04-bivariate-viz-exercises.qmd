...
-   What info do you need to set up the canvas? --> the axes variables 
-   What geometric layer (`geom_???`) might add these dots / points for each county? We haven't learned this yet, just take some guesses. --> point for scatter
...
# this sets the count and scale of the plot
ggplot(elections, aes(y = repub_pct_20, x = repub_pct_16))
```

```{r}
# Add a layer of points for each county
# Take note of the geom! --> point shows a scatter plot 
ggplot(elections, aes(y = repub_pct_20, x = repub_pct_16)) +
  geom_point()
```
...
#changes the shape to a different one! 
# shape = 4 is a x, 3 is a +, 5 is a diamond
```
...
#fill does nothing, color changes the point color
```
...
#we are adding the labels of the states to see which is which! 
```
...
-   outliers, ie, in what state do counties deviate from the national trend? Any ideas why this might be the case? --> there appears to be no outlier, maybe except DC in the bottom corner and a few texas ones, but generally they follow a linear relationship 
...
#median rent is a better predictor --> it gets stronger as time goes on 
...
this shows a stacked relationship and does not show more beyond the number of each party color and the year
...
red has more outliers, almost no outliers for blue, and purple has some outliers. red has the most republican support followed by purple and blue
...
# bad thing 1: the colors don't match the parties, and they overlap too much and we can't see where they overlap, we need it to be more opaque 
# bad thing 2: the y axis is very vague
```
...
# this makes the colors match the parties
```
...
# alpha changes the opacity so we can see where the full layers start and finish 
```
...
# facet wrap generates differnt plots/graphs for each party 
```
...
#this creates a stacked histogram in one plot 
```
...
-   Which do you like best? -->  I like the boxplots, it gives the most information (max/min/outliers/diff percentiles)
-   What is one pro of density plots relative to boxplots? --> density plots show more overlap between groups
-   What is one con of density plots relative to boxplots? --> you can't really see the specifics, it's more of a trend (less able to see statistical significance) 
...
#historical shows the x axes and fill defines the colors of the winners democrat/republican
```
...
pro: can visualize the split evenly between the different categories

con: not to scale, in the side by side one, you can clearly see that red has the highest count compared to blue, but proportional you cannot see the count 

#### Part b

What's your favorite bar plot from part and why?

I like the stacked plot the most becasue it's the easiest to interpret through comparison and count and it's to scale! 
